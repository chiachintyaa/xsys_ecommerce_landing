...
function CheckoutPage() {
    ...
    axios
      ...
      .then((res) => {
        ...
        setMidtransStatus(
          !!(
            res.data &&
            res.data.midtransPaymentInfo &&
            parseInt(res.data.midtransPaymentInfo.status) === 1
          )
        );
        setPaypalStatus(
          !!(
  ...
  const totalPrice = Math.round(subTotal && subTotal.reduce((prev, curr) => prev + curr));
  useEffect(() => {
            ...
            apiRequest.getOrderID(auth().access_token).then((result1) => {
              ...
              var body = {
                'transaction_details': {
                  ...
                  'gross_amount': Math.round(mainTotalPrice - discountCoupon),
                },
                'customer_details': {
  ...
  return (
    <>
      <Script
        ...
        // data-client-key="SB-Mid-client-msMRCcaLDf3KkUjN"
        data-client-key="Mid-client-amSixDGWI2E6IWxc"
        onLoad={() => {
                    ...
                    <div className="mt-[30px]">
                      <div className=" flex justify-between mb-5">
                        ...
                        >
                          {currency_icon
                            ? currency_icon +
                              Math.round(mainTotalPrice - discountCoupon)
                            : Math.round(mainTotalPrice - discountCoupon)
                          }
                        </p>
                      </div>
